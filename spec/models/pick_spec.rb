...
RSpec.describe Pick, type: :model do
  ...
  describe 'valid_pick' do
    ...
    context 'when user has 1 invalid pick' do
      before { Pick.create!(user:, season:, week:, team: denver_team, is_correct: false) }

      it 'allows another pick' do
        next_pick = Pick.new(user:, season:, week: week2, team: chicago_team)
  ...
  describe 'invalid pick' do
    ...
    context 'when pick is dup' do
      ...
      before do
        travel_to(week.start_date - 1.hour) do
          Pick.create!(user: user, season: season, week: week, team: team)
        end
      end
      ...
      it 'is invalid when the same user picks the same team again in the same season' do
        ...
        new_pick = Pick.new(user: user, season: season, week: week2, team: team)
    ...
    context 'when user has 2 invalid picks' do
      before do
        Pick.create!(user:, season:, week: week, team: denver_team, is_correct: false)
        Pick.create!(user:, season:, week: week2, team: green_bay_team, is_correct: false)
      end

      it 'is invalid when a user has already made the allotted number of incorrect picks' do
        new_pick = Pick.new(user: user, season: season, week: week3, team: chicago_team)
    ...
    context 'when user tries to make more than one pick in the same week' do
      before do
        travel_to(week.start_date - 1.hour) do
          Pick.create!(user:, season:, week:, team: chicago_team)
        end
      end

      it 'is invalid to create a second pick in the same week' do
        duplicate_pick = Pick.new(user:, season:, week:, team: denver_team)
